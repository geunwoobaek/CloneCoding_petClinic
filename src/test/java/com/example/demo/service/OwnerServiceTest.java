...
public class OwnerServiceTest {
	...
	void Init() {
		...
		pet3 = new Pet().builder().name("pet3").birthDate(LocalDate.of(2015, 12, 10)).build();
	...
	void OwnerTest() {
		// then
		ownerService.getOwnerList().stream().forEach(ownerdto -> System.out.println(ownerdto.toString()));
	}

	@Test
	@DisplayName("Owner 변경 테스트")
	void UpdateTest() {
		// given
		assert ownerService.findOneById(owner1.getId()).getFirstName().equals("geunwoo");
		assert ownerService.findOneById(owner1.getId()).getLastName().equals("Baek");
		// then
		owner1.setFirstName("Changed");
		owner1.setLastName("Name");
		ownerService.Join(owner1);
		assert ownerService.findOneById(owner1.getId()).getFirstName().equals("Changed");
		assert ownerService.findOneById(owner1.getId()).getLastName().equals("Name");
	}
